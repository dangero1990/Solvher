  ...
  return (
    <Card title='Solve a Partial Label Roll'>
      ...
      <form onSubmit={solve}>
        ...
        <input onChange={(e) => setCount(e.target.value)} type='number' inputMode='decimal' name='count' ref={input} step='any' pattern='[0-9]*' required />
        <label htmlFor='num-of-rolls'>3. Number of rolls</label>
        <input type='number' name='num-of-rolls' onChange={(e) => setRolls(e.target.value)} inputMode='decimal' step='any' pattern='[0-9]*' placeholder={rolls} ref={rollRef} />
